...
class Page {
	 ...
	 * %GoTo%			//跳转按钮
	 */
	private $pageType = '%first% %up%%F%%omitFA%%numberF%%omitEA%%E%%down%%ending% <b>共%pageToatl%页/%total%条</b>';
	//显示值设置
	 ...
	 * @return void
	 */
	public function __construct($total,$pageSize=10,$pageNumber=5,$pageParam='page'){
	 ...
	 */
	public function pageShow($pathinfo=false){
		$this->uri();
		if($this->pageToatl>1){
			if($this->page == 1){
		...
		$this->numberF();
		$pagestr =  str_ireplace(array('%pageToatl%','%page%','%pageSize%','%pageStart%','%pageStop%','%total%','%first%','%ending%','%up%','%down%','%input%','%GoTo%'),array($this->pageToatl,$this->page,$this->pageStop-$this->pageStart,$this->pageStart,$this->pageStop,$this->total,$first,$ending,$up,$down,$input,$GoTo),$this->pageType);
		if($pathinfo == true){
			$pagestr = preg_replace('/\/(\w+)-*\d*\.(\w+)\?\&page=(\d)/','$1-$3.$2', $pagestr);
		}
		return $pagestr;
	}
		...
		if($pageF!==false || $pageD!==false){
			if($pageF!==false){
				...
				for($i=$forStart;$i<$forStop;++$i){
					if($i==$this->page){
						$numberF .= '<a href="javascript:;" class="curPage">'.$i.'</a>';
					}else{
	...
	private function calculate(){
		...
		//$this->page = isset($_GET[$this->pageParam]) ? intval($_GET[$this->pageParam]) : '1' ;
		$this->page = !empty($args['page']) ? intval($args['page']) : 1;
